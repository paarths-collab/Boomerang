...
def fetch_sp500_tickers_and_save():
    ...
    try:
        ...
        print(f"[SUCCESS] Successfully saved {len(sp500_df)} S&P 500 stock symbols to '{output_path}'")
        ...
        if backup_path.exists():
            print(f"[INFO] Loading from local backup: {backup_path}")
            sp500_df = pd.read_csv(backup_path)
            sp500_df.to_csv(output_path, index=False)
            print(f"[SUCCESS] Backup list copied to '{output_path}'")
        else:
            print("[ERROR] No backup file found. Please provide a static CSV in /data.")
