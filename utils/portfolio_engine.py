...
def get_benchmark_data(ticker, start, end, initial_capital):
    ...
    Returns:
        pd.DataFrame: DataFrame with returns and equity curve
    """
...
def calculate_portfolio_metrics(equity_curve, start_date, end_date):
    ...
    Args:
        equity_curve (pd.Series): Series of portfolio equity values over time
        start_date (str): Start date of the backtest
        end_date (str): End date of the backtest
        
    Returns:
        dict: Dictionary containing calculated portfolio metrics
    """
...
def build_portfolio(orchestrator, tickers, market, start_date, end_date, strategies_config, initial_capital=100000):
    """
    Builds and evaluates a portfolio using multiple strategies on given tickers.
    
    Args:
        orchestrator: The orchestrator instance (not used directly but maintained for compatibility)
        tickers (list): List of ticker symbols to use in the portfolio
        market (str): Market identifier (e.g., 'US', 'india')
        start_date (str): Start date in 'YYYY-MM-DD' format
        end_date (str): End date in 'YYYY-MM-DD' format
        strategies_config (list): List of strategy names to use
        initial_capital (float): Initial capital for the portfolio (default: 100000)
        
    Returns:
        dict: Results containing equity curve, metrics, and other relevant data
    """
...
def run_portfolio_backtest(selections, start_date, end_date, initial_capital, market="US"):
    """
    Runs a portfolio backtest combining multiple strategies with specified weights.
    
    Args:
        selections (dict): Dictionary containing strategy selections with weights and parameters
        start_date (str): Start date in 'YYYY-MM-DD' format
        end_date (str): End date in 'YYYY-MM-DD' format
        initial_capital (float): Initial capital for the portfolio
        market (str): Market identifier for currency and benchmarks (default: "US")
        
    Returns:
        tuple: (portfolio_df, metrics, error_messages) - Portfolio data, calculated metrics, and any errors
    """
    all_equity_curves = {}
    error_messages = []
