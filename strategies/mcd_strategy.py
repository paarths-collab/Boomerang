...
def get_data(ticker, start, end, interval='1d'):
    """Fetches and prepares historical price data."""
...
def run_backtest(df, init_cash=100000):
    ...
    df['Equity_Curve'] = init_cash

    if df.empty:
        return pd.DataFrame(), []

    for i in range(len(df) - 1):
        # Update equity based on the closing price of the current day `i`
    ...
    df.loc[len(df) - 1, 'Equity_Curve'] = cash + (shares * df.loc[len(df) - 1, 'Close'])
...
def calculate_performance_metrics(backtest_df, trades_list, initial_capital):
    ...
    win_rate = 0
    if trades_list:
        # ... (win rate logic is correct and remains unchanged) ...
...
def run(ticker: str, start_date: str, end_date: str, initial_capital=100000, **kwargs) -> dict:
    ...
    try:
        ...
        # FIX: Pass initial_capital to the backtester
...
if __name__ == "__main__":
    ...
    if run_button:
        ...
        if "Error" in summary:
            ...
            fig.add_trace(go.Scatter(x=backtest_df.index, y=backtest_df['Close'], name='Price', line_color='skyblue'), row=1, col=1)
            # FIX: Use 'Equity_Curve' for plotting
            ...
            fig.add_trace(go.Scatter(x=buy_signals.index, y=buy_signals['Close'], mode='markers', name='Buy Signal', marker=dict(color='lime', size=10, symbol='triangle-up')), row=1, col=1)
            ...
            # MACD Chart (bottom subplot)
            fig.add_trace(go.Scatter(x=backtest_df.index, y=backtest_df['macd'], name='MACD', line=dict(color='blue')), row=2, col=1)
            fig.add_trace(go.Scatter(x=backtest_df.index, y=backtest_df['macd_signal'], name='Signal', line=dict(color='orange')), row=2, col=1)
...
#     if df.empty:
#         return pd.DataFrame()
#     # Robustly rename columns to prevent case-sensitivity issues
#     df.rename(columns={
#         "open": "Open", "high": "High", "low": "Low", "close": "Close", "volume": "Volume"
#     }, inplace=True, errors='ignore')
#     return df[['Open', 'High', 'Low', 'Close', 'Volume']]
...
#     df['signal'] = 0
#     df.loc[df['macd'] > df['macd_signal'], 'signal'] = 1
#     # Use diff() to find the exact crossover points for trading
#     df['trade'] = df['signal'].diff().fillna(0)
#     return df
...
#     # Loop from the first day to the second-to-last day
#     for i in range(len(df) - 1):
#         # Update equity based on the closing price of the current day `i`
...
#     # Calculate equity for the final day
...
#     if initial_capital == 0: return {} # Avoid division by zero
...
#                 yaxis=dict(title="Price"),
#                 yaxis2=dict(title="Equity", overlaying='y', side='right', showgrid=False),
#                 # Bottom subplot layout
#                 yaxis3=dict(title="MACD")
#             )
#             # Update the y-axis anchor for the second subplot
#             fig.update_yaxes(title_text="MACD", row=2, col=1)
#             st.plotly_chart(fig, use_container_width=True)
#         else:
#             st.warning("Backtest ran but did not produce any data.")
